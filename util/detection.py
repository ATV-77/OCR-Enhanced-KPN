...
class TextDetector(object):
    ...
    def get_contours(self, preds_KPN, kernel_postions=None, floodfill=False):
        ...
        for n in range(preds_KPN_bin.shape[0]):
            ...
            for dc in contours_list:
                # Fix for point format issue: ensure kernel_postions are in the correct format
                pt = (float(kernel_postions[0][n]), float(kernel_postions[1][n]))  # Convert to tuple of floats
                if cv2.pointPolygonTest(dc, pt, False) >= 0:
                    print("repeat_flag")
                    repeat_flag = True
                    break
            ...
            if len(contours):
                ...
                for i in range(len(contours)):
                    pt = (float(kernel_postions[0][n]), float(kernel_postions[1][n]))  # Convert to tuple of floats
                    if cv2.pointPolygonTest(contours[i], pt, False) >= 0:
    ...
    def detect(self, image, img_show, vis=False):
        ...
        if self.cfg.cuda:
            ...
            img = torch.ones((b, c, self.IN_SIZE[1], self.IN_SIZE[1]), dtype=torch.float32).to(device)
        else:
